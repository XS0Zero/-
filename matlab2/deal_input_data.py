...
def deal_input_data(data1 = None): 
    data = data1
    ...
    for i in np.arange(2,len(Xs)+1).reshape(-1):
        ...
        if alpha1 != alpha2 and phi1 != phi2:
            dx = ds * (np.cos(alpha1) - np.cos(alpha2)) / (alpha2 - alpha1) / (detphi) * (np.sin(phi2) - np.sin(phi1))
            dy = ds * (np.cos(alpha1) - np.cos(alpha2)) / (alpha2 - alpha1) / (detphi) * (np.cos(phi1) - np.cos(phi2))
            dz = ds / (alpha2 - alpha1) * (np.sin(alpha2) - np.sin(alpha1))
        else:
            if alpha1 == alpha2 and phi1 != phi2:
                dx = ds * np.sin(alpha1) / (detphi) * (np.sin(phi2) - np.sin(phi1))
                dy = ds * np.sin(alpha1) / (detphi) * (np.cos(phi1) - np.cos(phi2))
                dz = ds * np.cos(alpha1)
            else:
                if alpha1 != alpha2 and phi1 == phi2:
